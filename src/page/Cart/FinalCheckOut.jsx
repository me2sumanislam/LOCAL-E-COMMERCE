...
const FinalCheckoutPage = () => {
  ...
  // MyCart theke pathano selected data dhora
  ...
  if (selectedItems.length === 0) {
    return (
      <div className="h-screen flex flex-col items-center justify-center p-6 text-center bg-gray-50">
        <p className="text-gray-500 mb-4">আপনার কার্টে কোনো প্রোডাক্ট সিলেক্ট করা নেই!</p>
        <button onClick={() => navigate('/')} className="bg-orange-600 text-white px-8 py-3 rounded-xl font-bold">
  ...
  return (
    <div className="bg-gray-100 min-h-screen pb-32">
      ...
      <div className="bg-white mt-2 p-4">
        <h2 className="font-bold text-gray-800 mb-4 border-b pb-2">প্রোডাক্ট লিস্ট ({selectedItems.length})</h2>
        {selectedItems.map((item) => (
          <div key={item.id} className="flex gap-4 py-4 border-b last:border-0">
            <img src={item.image} className="w-20 h-20 object-cover rounded-lg" alt="" />
            <div className="flex-1">
              <h3 className="text-sm text-gray-700 line-clamp-2">{item.title}</h3>
              <div className="flex justify-between mt-2 font-bold items-center">
                <span className="text-orange-600 text-lg">৳{item.price}</span>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-4 flex justify-between items-center z-50 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
        ...
        <button className="bg-orange-600 text-white px-10 py-4 rounded-xl font-bold text-lg active:scale-95 transition-all shadow-lg shadow-orange-100">
          অর্ডার করুন
        </button>
      </div>
    </div>
